/* ============================================================
   ANDROID SCRIPT - Material Design Logic
...
function saveCart() {
    try {
        localStorage.setItem('android_cart', JSON.stringify(cart));
    } catch (_) {}
    updateBadges();
}
...
function loadCart() {
    try {
        const d = localStorage.getItem('android_cart');
...
function saveOrders() {
    try {
        localStorage.setItem('android_orders', JSON.stringify(orders));
    } catch (_) {}
}

function loadOrders() {
    try {
        const d = localStorage.getItem('android_orders');
...
function renderHome() {
    ...
    container.innerHTML = `
        ...
        <div style="background:var(--card);border-radius:var(--radius);padding:16px 18px;box-shadow:var(--shadow-sm);margin-bottom:16px;">
            <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:space-between;">
                ...
                <button onclick="switchTab('profile')" 
                        style="font-size:13px;font-weight:600;color:var(--primary);background:none;border:none;cursor:pointer;font-family:inherit;padding:4px 0;">
                    Learn more →
                </button>
            </div>
        </div>
...
function renderProfile() {
    ...
    container.innerHTML = `
     ...
     <div style="background:var(--card);border-radius:var(--radius);padding:16px 18px;box-shadow:var(--shadow-sm);font-size:13px;color:var(--text-tertiary);text-align:center;margin-bottom:16px;">
        ...
        <div style="background:var(--card);border-radius:var(--radius);padding:14px 16px;box-shadow:var(--shadow-sm);font-size:12px;color:var(--text-tertiary);text-align:center;border-left:3px solid var(--primary);">
...
function switchTab(tabId) {
    ...
    // Update tab buttons
    document.querySelectorAll('.android-nav button').forEach(btn => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Log
    console.log('🤖 Android Interface loaded');
